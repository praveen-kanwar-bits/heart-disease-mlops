...
REQUEST_ERROR_COUNT = Counter("api_request_errors_total", "Total request errors", ["endpoint", "error_type"])
PREDICTION_COUNT = Counter("model_predictions_total", "Total predictions made", ["prediction", "model_version"])
...
async def lifespan(app: FastAPI):
    ...
    if metadata_path.exists():
        with open(metadata_path, "r") as f:
            meta = json.load(f)
            app.state.threshold = float(meta.get("decision_threshold", settings.values["api"]["confidence_threshold"]))
    else:
